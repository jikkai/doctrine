...
The shell exposes `header`, `header-inner`, `brand`, `navigation`, `sidebar`, `main`, `content`, and
`footer`. Built-ins expose `badge`, `callout`, `card`, `card-grid`, `code-block`,
...
`@amamo/doctrine/components` for custom wrappers.
...
```mdx
<CardGrid>
  <Card title="Installation">Start with [Getting started](/guide/getting-started/).</Card>
