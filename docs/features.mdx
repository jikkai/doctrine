...
Navigation never falls back between locale files. Localized site text falls back from the current
locale to the default locale, then to the first configured value. Locale switch labels fall back to
the locale code.
...
Doctrine ships precompiled interface CSS and built-in `Badge`, `Callout`, `Card`, `CardGrid`, `Step`,
`Steps`, `Tab`, and `Tabs` components. A configured CSS file loads after the defaults. A custom
component map is merged after built-ins and is shared by SSR and browser hydration.
...
`doctrine.config.*`, navigation modules, and custom components are executable modules. They can run
during config loading, bundling, SSR, or hydration. Build only sources you trust with the privileges
of the build machine and deployed site.
