...
| 选项          | 类型                               | 默认值                             | 用途                                   |
| ------------- | ---------------------------------- | ---------------------------------- | -------------------------------------- |
| `title`       | `string \| Record<string, string>` | `"Documentation"`                  | 品牌文案和页面 title 后缀。            |
| `description` | `string \| Record<string, string>` | `"Documentation built from MDX."`  | 默认 meta description。                |
| `siteUrl`     | `string`                           | `"http://localhost/"`              | 绝对公开 URL 和 Vite 部署 base。       |
| `githubUrl`   | `string`                           | 无                                 | Header 中 GitHub 图标的外部链接。      |
| `copyright`   | `string \| Record<string, string>` | 无                                 | 可本地化的 footer 文案。               |
...
`title`、`description` 和 `copyright` 可以是所有 locale 共用的字符串，也可以是 locale 映射。映射
按“当前 locale → `locales.default` → 第一个值”回退。

`siteUrl` 必须使用 HTTP 或 HTTPS，且不能包含 query 或 fragment。Doctrine 会把 pathname 归一化
为一个开头斜杠和一个结尾斜杠；这个 pathname 就是 `base`。`githubUrl` 也必须使用 HTTP 或
HTTPS。
...
根目录的 `title` 可省略；嵌套导航文件必须提供非空 title。`page` 必须是不含斜杠、扩展名和
locale 后缀的直属 MDX basename。`directory` 必须是直属子目录名，不能是 `.` 或 `..`。
...
还会启用 Tailwind Vite 插件。详见[自定义](/zh-CN/customization/)。
