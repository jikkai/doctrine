...
export function Step({ children, className, title, ...props }: IStepProps) {
  return (
    <li className={`relative pb-7 pl-7 last:pb-0 ${className ?? ''}`} {...props} data-slot="step">
      <span
        aria-hidden="true"
        className="absolute top-0 -left-3.5 flex size-7 items-center justify-center rounded-full border border-primary/45 bg-background text-xs font-semibold text-primary before:content-[counter(list-item)]"
      />
...
export function Steps({ className, ...props }: IStepsProps) {
  return (
    <ol
      className={`ml-4 list-none border-l border-border/60 pl-0 ${className ?? ''}`}
      {...props}
      data-slot="steps"
    />
  )
